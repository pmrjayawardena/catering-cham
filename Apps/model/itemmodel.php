...
class item{
    ...
    public function  addItem($arr){
        global $con;
        $r=$con->prepare("INSERT INTO item(item_name,cat_id,item_price,item_des,item_status,item_size,suitable_for)VALUES(?,?,?,?,?,?,?)");

        print_r($arr);
        $r->execute(array($arr['item_name'],$arr['cat_id'],$arr['item_price'],$arr['item_des'],"Available",$arr['size_name'],$arr['suitable_for']));
